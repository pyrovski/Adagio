...
cd $path
cores=$SLURM_NPROCS
mpirun -n $SLURM_NPROCS MV2_CPU_BINDING_POLICY=bunch $trace -genv OMPI_MCA_gmpi_algo adagio $oldDir/$command >log 2>errlog

#cores=32
#mpirun -n $cores MV2_CPU_BINDING_POLICY=scatter $trace -genv OMPI_MCA_gmpi_algo adagio -genv OMPI_MCA_gmpi_bind collapse $oldDir/harness -S >log 2>errlog
