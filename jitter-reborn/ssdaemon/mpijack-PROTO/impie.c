...
// Settings:
//   0 - Disable the setting of power state (used to find overhead of jitter).
//   1 - Use Mark's lpagent to change CPU gear
//   2 - Use Tyler's ssdaemon to change CPU gear
#define POWERSTATE_SETTING 2

#if POWERSTATE_SETTING==1
#include "mark.h"
#elsif POWERSTATE_SETTING==2
#include "sscaling.h"
#endif
...
#if POWERSTATE_SETTING==1
#define setCPUgear(n) if (agent_set_pstate(0, (n))) { fprintf(STDERR,"Unable to set pstate with lpagent!\n"); exit(1); }
#elsif POWERSTATE_SETTING==2
#define setCPUgear(n) if (ss_setGear((n),1)) { fprintf(STDERR,"Unable to set pstate with ssdaemon!\n"); exit(1); }
#else
#define setCPUgear(n) 0
#endif
...
#endif // POWERSTATE_SETTING==1
		setCPUgear(0);
#endif // POWERSTATE_SETTING
	}
...
int MPI_Comm_rank(MPI_Comm comm, int *rank) {
	...
	if (check++==0) {
		...
		/*
		printf("[%d] JITTER is ENABLED! (DEBUG_OUTPUT=%s, JITTER_ALLREDUCE=%s POWERSTATE_SETTING=%s)\n",
			   RANK,
			   DEBUG_OUTPUT?"TRUE":"false",
			   JITTER_ALLREDUCE?"TRUE":"false",
			   POWERSTATE_SETTING?"TRUE":"false"
			  );
		*/
